...
- Cookie 字符串：`userId=xxx; serviceToken=xxx`（可选再带 `ssecurity`、`passToken`、`deviceId`）。可直接粘贴浏览器或抓包工具复制的完整 `Cookie:` 请求头，服务端会自动提取字段并忽略其他 Cookie。若凭证来自 `https://account.xiaomi.com/`，请连同同一组 `passToken` 一起粘贴；服务端会先用它换取 Mina 专用 `micoapi` serviceToken，再验证设备。也可通过抓包小爱音箱 App / `api2.mina.mi.com` 请求获得已经是 Mina 专用的 Cookie。
